'''Script for training and testing a CNN on hdf5 data containing 
3 types of events detected by a 48x48 CHEC camera, done by analysing the centre 32x32 image
to ignore any corner defects. .'''
...
from net_utils_no_norm import *
import time

runname='muon100_no_norm'
...
for i, a in enumerate(attributions):
    ...
    print(xs[i])
    print('x2',x2[i])
    newplot(x2[i].reshape(32, 32), xi=None, axis=axes[row,col*2]).set_title('Original')
    plot(a.reshape(32,32), xi=None, axis=axes[row,col*2+1]).set_title('Attributions, label='+str(int(ys[0,i])))
