...
def generate_training_sequences(onlyfiles,batch_size, batchflag, shilonflag=False):
    ...
    while True:
        for file in filelist:
            ...
            trainarr = np.reshape(trainarr, (notrigs, 4, 48, 48, 1))
            chargearr = np.reshape(chargearr, (notrigs, 4, 48, 48, 1))
            rtarr = np.reshape(rtarr, (notrigs, 4, 48, 48, 1))
            ftarr = np.reshape(ftarr, (notrigs, 4, 48, 48, 1))
            fwhmarr = np.reshape(fwhmarr, (notrigs, 4, 48, 48, 1))
            amparr = np.reshape(amparr, (notrigs, 4, 48, 48, 1))
            meanarr = np.reshape(meanarr, (notrigs, 4, 48, 48, 1))
            rmsarr = np.reshape(rmsarr, (notrigs, 4, 48, 48, 1))
            ...
            ta2 = np.zeros((training_sample_count, 4, 48, 48, 1))
